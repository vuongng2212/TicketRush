...
*   **Danh sách Task chi tiết:**
    *   [x] **Task 4.1:** Định nghĩa file protobuf `ticket.proto` và cấu hình Maven plugin tự động compile sang Java classes.
    *   [x] **Task 4.2:** Viết gRPC Server Service xử lý check-in vé trên Spring Boot.
    *   [x] **Task 4.3:** Xây dựng giao diện Web Admin Scanner trên Next.js (yêu cầu quyền Camera, quét QR Code) và gọi API Route (đóng vai trò gRPC Client) kết nối tới gRPC Server của backend.
    *   [x] **Task 4.4:** Viết script load test k6 giả lập 5,000 requests đặt vé đồng thời.
    *   [x] **Task 4.5:** Thực hiện load test, đo đạc chỉ số RPS, latency p95/p99 và lưu kết quả vào benchmark report.
*   **Tiêu chí hoàn thành (DoD):**
