...
1.  **Next.js 14 Web Client:** Chạy môi trường Node.js phía Client, giao tiếp qua GraphQL HTTP (Queries/Mutations) và WebSockets (Subscriptions).
    *   **Web Admin Scanner (Next.js Routing):** Tích hợp camera quét QR Code thông qua trình duyệt thiết bị di động. Khi nhận diện mã vé, Next.js server-side đóng vai trò là **gRPC Client** thực hiện gRPC call đến backend.
2.  **Spring Boot Monolith Application:** Chạy trên môi trường JVM 21, lắng nghe trên 2 cổng:
    *   **Port 8080:** HTTP/WebSocket Server phục vụ GraphQL API (Spring for GraphQL).
...
### 3.3 Quy trình Soát Vé gRPC (gRPC Check-in Sequence)
Mô hình soát vé tốc độ cao sử dụng gRPC tích hợp Next.js và Spring Boot:

```mermaid
sequenceDiagram
    autonumber
    actor Gatekeeper as Nhân viên soát vé
    participant Mobile as Trình duyệt di động (Camera)
    participant NextServer as Next.js API (gRPC Client)
    participant Spring as Spring Boot gRPC Server
    participant Postgres as PostgreSQL DB

    Gatekeeper->>Mobile: Mở trang Web Admin Scanner & Quét mã QR vé
    Mobile->>NextServer: POST /api/checkin { ticketCode: "qr_code_hash_123" }
    NextServer->>Spring: gRPC VerifyTicket(ticket_code: "qr_code_hash_123")
    Spring->>Postgres: SELECT * FROM tickets WHERE ticket_code = 'qr_code_hash_123'
    alt Vé hợp lệ & Chưa check-in
        Postgres-->>Spring: Vé OK (check_in_status = false)
        Spring->>Postgres: UPDATE tickets SET check_in_status = true, check_in_time = NOW()
        Spring-->>NextServer: gRPC Response: valid = true, seat_number = "VIP A1", user_name = "Nguyen Kasim"
        NextServer-->>Mobile: HTTP 200 { valid: true, data: ... }
        Mobile-->>Gatekeeper: Hiện màn hình XANH: Vé Hợp lệ. Mời vào!
    else Vé không hợp lệ hoặc Đã check-in trước đó
        Postgres-->>Spring: Lỗi (đã check-in hoặc không tồn tại)
        Spring-->>NextServer: gRPC Response: valid = false, message = "Vé đã được sử dụng"
        NextServer-->>Mobile: HTTP 200 { valid: false, message: ... }
        Mobile-->>Gatekeeper: Hiện màn hình ĐỎ: Cảnh báo vé đã sử dụng!
    end
```
